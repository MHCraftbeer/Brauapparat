...
//Wenn der Programmspeicherplatz mit mehr als 85% belegt wird funktioniert das heizungsrelais nicht mehr, wohl wegen den unsigend longs die überlaufen
//mit lCD Keypad shield sind alle nutzbaren Pins vom Uno belegt. mehr geht nicht.
 ...
 * 2.06 GoBetwino integriert (Arduino sendet über USB LogDaten an gobetwino.exe und speichert sie in Datei; http://www.mikmo.dk/gobetwino.html),
 * globale Variablen nachvollziehbarer gemacht, Changelog erstellt, allg. Verbesserungen
 *2.07 paar Bugs entfernt: LED Takt OK, Buzzer Takt OK;
 *2.08 LCD Keypad Shield Steuerung wieder eingebaut, Variablen im Sketch nach oben gestelllt
 *Keypad funktioniert, Manueller Modus bis auf Timer auch schon
 ...
 *2.43 ausprobieren einer pid regelung: http://playground.arduino.cc/Code/PIDLibrary
 *2.50 alles funktioniert. Programmspeicher darf 85% nicht überschreiten, sonst funktioniert das relais nicht mehr
AB HIER NUR NOCH EINSTELLUNGEN ÄNDERN; KEINE NEUE ZEILE CODE MEHR; KEINE NEUEN VARIABLEN
 *
 *
 *TODO:
 *-Heizungsregelung einstellen dh. richtigen Kwert finden. Temperaturmessung kalibrieren dh. Referenzwiderstand richtig einstellen. 
 ...
 *-min und max werte an reale werte anpassen 
...
const byte ruehrerPin2 = A4;
const byte heizungPin = A3;
//buzzer
const byte buzzerPin = 2;
//leds (einfach über bsp "blinkwithoutdelay" bestimmen)
const byte ruehrerLed1 = 11;
const byte ruehrerLed2 = 12;
const byte heizungLed = 3;
const byte programmLed = 13; //besonders schön: die led blinkt beim Upload
//////////////////////////////
///////////////////////////////
///////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//
//////////////////////////////////////////////////////////////////////////////Eingabe
byte tempMin = 25; //minimal Temperatur
byte tempMax = 99;
byte timerMin = 2; //minimal Timer
byte timerMax = 180;
int timerKochenMin = 2;
byte tempKochenMin = 25;
              ...
              buttonPressed = true;
                ...
                switch(autoKochenMenue) {
                  ...
                  case 2: {  //hoch von tempKochen
                    ...
                    else tempKochen = tempMax;
                    break;
                  }
                  case 3: { // hoch von timerstep
                    if (timerKochen > timerMin) {
                      if (millis() - zaehlerStart >= zaehlerTakt) {
                      if (!buttonPressed) zaehlerStart = millis();
                      timerKochen--;
                      }
...
float tempT() { //Temperatur am Topfboden
  ...
  iaverage = 9465.0 / iaverage; //Referenzwiderstand
...
float igradient; 
  if (autoMaischenStart || autoKochenStart || manuellStart) {
      if (millis() - heizungVorherigeZeit >= heizungMesszyklus) {  //berechnet Gradienten in K/min
              igradient = ((tempT() - heizungVorherigeTemp) / (millis() - heizungVorherigeZeit) * 60000.0); //iGradient ist der Temperaturanstieg des Behälters in K/min
              if (igradient < 0) igradient = 0.1;
              heizungVorherigeZeit = millis();
              heizungVorherigeTemp = tempT();
      }
      if (tempT() > tempSoll - 0.5) {
        tempSensorDaempfung++;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////LED Takt/////////////////////////////////////////////////////////////////////
/////////////////////////////////////für programmLed und 
boolean ledTakt() {
...
void autoMaischen() {
  ...
  if (autoMaischenStart && !manuellStart && !autoKochenStart) {  //hier startet es erst richtig
    ...
    if (autoMaischenEnde){  //Schaltet alles auf Anfang wenn die Automatik vorbei ist, beendet sie und aktiviert den läuteralarm
      ...
      tempStep1 = tempMin;   //wieder die StartTemperaturen
      tempStep2 = tempMin;
      tempStep3 = tempMin;
      tempStep4 = tempMin;
      tempStep5 = tempMin;
      tempStep6 = tempMin;
      timerStep1 = timerMin;  // wieder die StartTimer Min
      timerStep2 = timerMin;
      timerStep3 = timerMin;
      timerStep4 = timerMin;
      timerStep5 = timerMin;
...
void hopfentimer(){
  ...
  if (timerHopfenStart && tempSollErreicht && !timerHopfenEnde) { //Verstellt den Timer wenn Automatik 
    ...
    if (icurrentHopfenTimerMillis - timerHopfenPreviousMillis >= 1000) {
       timerHopfenSet--;
  ...
  if (autoKochenStart && !manuellStart && !autoMaischenStart) {  //programmanfang
      ...
      if (hopfen <= letzterHopfen && !keinHopfenMehr) {
        ...
        if (autoKochenEnde){  //Schaltet alles auf Anfang wenn die Automatik vorbei ist
          ...
          meldung = false;
          datenUebergeben = false;
          timerHopfen1 = timerMin;
          timerHopfen2 = timerMin;
          timerHopfen3 = timerMin;
          timerHopfen4 = timerMin;
          timerHopfen5 = timerMin;
          maximaKochen = false;
          tempKochen = tempKochenMin;
          meldung = false;
    ...
    if (hMenue == 1) { 
      switch(manuellMenue) {
        ...
        case 5: {  //Übersicht während des Programms
          ...
          lcd.write(byte(1));lcd.print("C ");
          lcd.write(byte(6));
          lcd.print(tempSoll);lcd.print(".00");
          lcd.write(byte(1));lcd.print("C ");
          break;
        }
      }
    ...
    if (hMenue == 2) { ////////////////////////////////////////////////////////////////////////////////AutoMaischen Modus
      ...
      if (autoMaischenStart) {  //////////////////////////////////////////Die StatusDisplays während des Automaischens
        if (autoMaischenStatus == 1) {  //allgemeine Übersicht, istTemp, sollTemp, verbleibende Zeit
          ...
          lcd.print(tempT());lcd.write(byte(1));lcd.print("C");
          lcd.write(byte(6));
          lcd.print(tempSoll);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
          lcd.write(byte(5));
        }
              ...
              switch(currentStep) {
                case 1:{
                  ...
                  lcd.write(byte(6));lcd.print(tempStep2);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
                  lcd.write(byte(5));
                  break;
                }
                case 2:{
                  ...
                  lcd.setCursor(0,1);
                  lcd.print("        "); 
                  lcd.write(byte(6));lcd.print(tempStep3);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
                  lcd.write(byte(5));
                  break;
                }
                case 3:{
                  ...
                  lcd.setCursor(0,1);
                  lcd.print("        "); 
                  lcd.write(byte(6));lcd.print(tempStep4);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
                  lcd.write(byte(5));
                  break;
                }
                case 4:{
                  ...
                  lcd.setCursor(0,1);
                  lcd.print("        "); 
                  lcd.write(byte(6));lcd.print(tempStep5);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
                  lcd.write(byte(5));
                  break;
                }
                case 5:{
                  ...
                  lcd.setCursor(0,1);
                  lcd.print("        "); 
                  lcd.write(byte(6));lcd.print(tempStep6);lcd.print(".00");lcd.write(byte(1));lcd.print("C");
                  lcd.write(byte(5));
                  break;
                }
              }
...
void gobetwino() {
    char buffer[20];
      if (!logGestartet) {
        ...
        Serial.print(";");
        Serial.print("IstTemp*100");
        Serial.print(";");
        Serial.print("SollTemp*100");
        Serial.print(";");
    ...
    if (millis() - logPreviousMillis >= gobetwinoIntervall) {
      ...
      logTime+=(gobetwinoIntervall/1000);
       Serial.print("#S|LOGTEST|[");
        ...
        Serial.print(";");
        Serial.print(itoa((tempT()*100), buffer, 10)); //temperatur Topfboden
        Serial.print(";");
        Serial.print(itoa(tempSoll*100, buffer, 10));
        Serial.print(";");
